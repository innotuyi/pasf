<?php session_start();  
  ...
  if (isset($_POST['userData'])  && isset($_POST['EditFname'])     && isset($_POST['EditPhone']) && isset($_POST['EditEmail'])  ) {
    ...
    if (!preg_match("/^[a-zA-Z ]*$/",$EditFname)) {
      ...
      $sql = $con->prepare("UPDATE tbl_admin SET tbl_adminName  =?, tbl_adminEmail =?,  tbl_adminTel  =?, tbl_adminAddress =?, tbl_adminPass =?  WHERE tbl_adminId =? " );
  ...
  if (isset($_POST['username'])  && isset($_POST['EditFname'])     && isset($_POST['EditPhone']) && isset($_POST['EditEmail'])  ) {
    ...
    if($sql->rowCount() > 0){
      ...
      $sql = $con->prepare("INSERT INTO tbl_admin (tbl_adminName, tbl_adminEmail, tbl_adminTel, tbl_adminAddress, tbl_adminUserName, tbl_adminPass, tbl_adminStatus) VALUES (?,?,?, ?,?,?, ?) " );
