<?php session_start();  
  ...
  if (isset($_POST['userData'])  && isset($_POST['EditFname'])     && isset($_POST['EditPhone']) && isset($_POST['EditEmail'])  ) {
    ...
    if (!preg_match("/^[a-zA-Z ]*$/",$EditFname)) {
      ...
      $sql = $con->prepare("UPDATE tbl_finance SET tbl_financeName  =?, tbl_financeEmail =?,  tbl_financeTel  =?, tbl_financeAddress =?, tbl_financePass =?  WHERE tbl_financeId =? " );
  ...
  if (isset($_POST['username'])  && isset($_POST['EditFname'])     && isset($_POST['EditPhone']) && isset($_POST['EditEmail'])  ) {
    ...
    if($sql->rowCount() > 0){
      ...
      $sql = $con->prepare("INSERT INTO tbl_finance (tbl_financeName, tbl_financeEmail, tbl_financeTel, tbl_financeAddress, tbl_financeUserName, tbl_financePass, tbl_financeStatus) VALUES (?,?,?, ?,?,?, ?) " );
